...
async function KuaiQuCaiPullWrite(joint, uqIn, data) {
    ...
    try {
        ...
        if (postResult.CODE != 200) {
            logger_1.logger.error('KuaiQuCaiPush Fail: {Code:' + postResult.CODE + ',PackageId: ' + body["COMPANY_SALE_NO"] + ',Type:' + postOptions.path + ',Datetime:' + Date.now() + ',Message:' + optionData + '}');
            result = false;
        }
        else {
