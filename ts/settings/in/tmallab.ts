...
let pullSql = ` SELECT  r.ID, m.name as BrandName, p.OriginalId, zcl_mess.dbo.fn_mi_pack_toString(j.packnr,j.quantity,j.unit,'abstract') as Package, 
...
// AND EXISTS ( SELECT 1 FROM zcl_mess.dbo.ProductsMarketing WHERE marketingid = 'A02-20200416A' AND JKCat = PackageId )
