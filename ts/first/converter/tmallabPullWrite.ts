...
// import { Joint, UqInTuid, UqIn, Tuid, MapUserToUq } from "../../uq-joint";
import _, { now, round } from 'lodash';
    ...
    try {
        ...
        // 产品 “新增”或者“修改”的情况。需要批量推送。先判断单个数据对应的处理情况，存储到数组中（等存储够一定量的数据批量推送）。
            ...
            if (templateTypeId == 1) {
                ...
                GlobalVar.addOrEditList_bio.push(AddOrEditFormat);
                console.log('bio count: ' + GlobalVar.addOrEditList_bio.length);
            } else if (templateTypeId == 3) {
                GlobalVar.addOrEditList_cl.push(AddOrEditFormat);
        ...
        if (GlobalVar.addOrEditList_chem.length > 99) {
            ...
            GlobalVar.addOrEditList_chem.forEach(element => {
                productList_addOrEdit.push(element);
            });
            GlobalVar.addOrEditList_chem = [];  //清空数组
            let addData = {
        ...
        if (GlobalVar.addOrEditList_bio.length > 9) {
            console.log(format(Date.now(), 'yyyy-MM-dd HH:mm:ss') + ' 生物试剂数量: 10 ，准备推送... , 仪器耗材数量: ' + GlobalVar.addOrEditList_cl.length + ', 化学试剂数量: ' + GlobalVar.addOrEditList_chem.length);

            let productList_addOrEdit: any = [];

            GlobalVar.addOrEditList_bio.forEach(element => {
                productList_addOrEdit.push(element);
            });
            GlobalVar.addOrEditList_bio = [];
        ...
        if (GlobalVar.addOrEditList_cl.length > 9) {
            ...
            GlobalVar.addOrEditList_cl.forEach(element => {
                productList_addOrEdit.push(element);
            });
            GlobalVar.addOrEditList_cl = [];
...
function GetAddOrEditFormat(itemNum, brand, packingSpecification, casFormat, catalogPrice, descriptionC, description, descriptionST, purity, storage, jkid,
    templateTypeId, mdlNumber, packnr, unit, delivetime, salePrice, startTime, endTime, activeDiscount, PackageId) {
